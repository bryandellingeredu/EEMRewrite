...
namespace Application
{
    public class WorkflowHelper
    {
        ...
        private readonly List<string> _svtcRooms = new List<string>
        {
        ...
        private readonly List<string> _cioRooms = new List<string>
        {
            "Bldg650CollinsHallBSAPConferenceRoomSVTC@armywarcollege.edu",
            "Bldg650CollinsHallB033ASVTC@armywarcollege.edu",
            "Bldg650CollinsHallB030@armywarcollege.edu",
        ...
        internal async Task SendNotifications()
        {
            ...
            if (_activity.DistinguishedVisitorNotificationToExecServices && !_activity.DistinguishedVisitorNotificationToExecServicesNotificationSent) await this.SendDistinguishedVisitorNotificationToExecServices();
            if (_activity.DistinguishedVisitorAttending && !_activity.DistinguishedVisitorAttendingNotificationSent) await this.SendDistinguishedVisitorNotificationToExecServices();
        }
        ...
        private async Task sendCIORepsNotification()
        {
            string title = $"{await GetLocation()} has been selected";
            string body = $"<p>{await GetLocation()} has been selected, please ensure coordination with the CIO to ensure IT capabilities in this room are available </p>";
            ...
            List<string> Emails = GetEmails("CIOReps").ToList();
            if (!string.IsNullOrEmpty(_activity.LastUpdatedBy))
            {
                Emails.Add(_activity.LastUpdatedBy);
            }
            else
            {
               Emails.Add(_activity.CreatedBy);
            }

            await GraphHelper.SendEmail(Emails.ToArray(), title, body);
            var activityToUpdate = await _context.Activities.FindAsync(_activity.Id);
            activityToUpdate.CIORepsNotificationSent = true;
            _context.Activities.Update(activityToUpdate);
            await _context.SaveChangesAsync();

        }
        private async Task sendSecretNotification()
        {
            string title = $"An event level clearance of  {_activity.EventClearanceLevel} has been selected for {await GetLocation()}  ";
            string body = $"<p>An event level clearance of  {_activity.EventClearanceLevel} has been selected for {await GetLocation()} Please coordinate with USAWC G2 to ensure proper security procedures are followed for this {_activity.EventClearanceLevel} event.</p>";
            body = body + $"<p><strong> Event Title: </strong> {_activity.Title} </p>";
            body = body + $"<p><strong> Room: </strong> {await GetLocation()} </p>";
            body = body + $"<p><strong> Start Time: </strong> {GetStartTime()} </p>";
            body = body + $"<p><strong> End Time: </strong> {GetEndTime()} </p>";
            ...
            List<string> G2Emails = GetEmails("G2").ToList();
            if (!string.IsNullOrEmpty(_activity.LastUpdatedBy))
            {
                G2Emails.Add(_activity.LastUpdatedBy);
            }
            else
            {
                G2Emails.Add(_activity.CreatedBy);
            }

            await GraphHelper.SendEmail(G2Emails.ToArray(), title, body);
            var activityToUpdate = await _context.Activities.FindAsync(_activity.Id);
            activityToUpdate.SecretNotificationSent = true;
            _context.Activities.Update(activityToUpdate);
            await _context.SaveChangesAsync();
        }

        private async Task sendSVTCNotification()
        {
            ...
            body = body + $@"<p><p/><p><p/><p> To view in the Enterprise Event Manager (EEM), click the: <a href='{_settings.BaseUrl}?id={_activity.Id}&categoryid={_activity.CategoryId}'> EEM Link </a></p>
              <p></p><p></p><p>DO NOT REPLY TO THIS E-MAIL. THIS MESSAGE WAS AUTOMATICALLY GENERATED BY THE SYSTEM AND IS NOT MONITORED.</p>";
            if(!string.IsNullOrEmpty(_activity.LastUpdatedBy)) {
                await GraphHelper.SendEmail(new[] { _activity.LastUpdatedBy }, title, body);
            }
            else
            {
            ...
            var activityToUpdate = await _context.Activities.FindAsync(_activity.Id);
            activityToUpdate.SVTCNotificationSent = true;
            _context.Activities.Update(activityToUpdate);
            await _context.SaveChangesAsync();
        }

        private async Task sendRoomResourceNotification()
        {
            string[] emailsToCheck = {  "Bldg650CollinsHallNormandyConferenceRoomSVTC@armywarcollege.edu",
            ...
            bool containsAny = emailsToCheck.Any(email => _activity.RoomEmails.Contains(email, StringComparer.OrdinalIgnoreCase));
            if (containsAny) {
                string title = $"A room resource is required for {_activity.Title} at {GetStartTime()}";
                string body = $@"<p> {_activity.CreatedBy} has requested Room Resources  for the event {_activity.Title} at {await GetLocation()} </p>"
                ...
                body = body + $"<p><strong> Start Time: </strong> {GetStartTime()} </p>";
                body = body + $"<p><strong> End Time: </strong> {GetEndTime()} </p>";
                body = body +  $"<p><strong> Resources Required: </strong>";

                                    
                if (_activity.RoomResourceSipr) body = body + "SIPR, ";
                if (_activity.RoomResourceNipr) body = body + "NIPR, ";
                if (_activity.RoomResourceRen) body = body + "REN, ";
                if (_activity.RoomResourceNts) body = body + "NTS, ";
                if (_activity.RoomResourceNtg) body = body + "NTG, ";
                if (_activity.RoomResourceOther) body = body + _activity.RoomResourceOtherText;

                body = body + "</p>";

                body = body + $@"<p><p/><p><p/><p> To view in the Enterprise Event Manager (EEM), click the: <a href='{_settings.BaseUrl}?id={_activity.Id}&categoryid={_activity.CategoryId}'> EEM Link </a></p>
              <p></p><p></p><p>DO NOT REPLY TO THIS E-MAIL. THIS MESSAGE WAS AUTOMATICALLY GENERATED BY THE SYSTEM AND IS NOT MONITORED.</p>";
                await GraphHelper.SendEmail(GetEmails("Room Resource"), title, body);
            }
            var activityToUpdate = await _context.Activities.FindAsync(_activity.Id);
            activityToUpdate.RoomResourceNotificationSent = true;
            _context.Activities.Update(activityToUpdate);
            await _context.SaveChangesAsync();
        }
        ...
        private async Task SendDistinguishedVisitorNotificationToExecServices()
        {
             string title = $"DV Notification to Exec Services for event {_activity.Title}";

             string body = $@"<p> Executive Services,</p><p></p>
                        DV Notification to Exec Services for event {_activity.Title}";
            ...
            var activityToUpdate = await _context.Activities.FindAsync(_activity.Id);
            activityToUpdate.DistinguishedVisitorNotificationToExecServicesNotificationSent = true; 
            activityToUpdate.DistinguishedVisitorAttendingNotificationSent = true; 
            _context.Activities.Update(activityToUpdate);
            await _context.SaveChangesAsync();
        ...
        private async Task SendOfficerRequestedNotification(string officerType, DateTime? startDate, DateTime? endDate)
        {
            ...
            {
                // When running locally, always send to this email
                emails =  new[] { _settings.LocalNotificationEmail};
            }
