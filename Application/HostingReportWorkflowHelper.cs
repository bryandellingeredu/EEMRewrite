...
namespace Application
{
    public class HostingReportWorkflowHelper
    {
        ...
        public HostingReportWorkflowHelper(Activity activity, Settings settings, DataContext context, HostingReport hostingReport, IWebHostEnvironment webHostEnvironment)
        {
            _activity = activity;
            _settings = settings;
        ...
        internal async Task SendNotifications()
        {
            ...
            if (!_hostingReport.HostingReportLocalTransportationNotificationSent && _hostingReport.ReportType == "Hosting Report" && _hostingReport.LocalTransportationNeeded) await this.SendHostingReportLocalTransportationNotifiaction();
        }
        ...
        private async Task SendHostingReportLocalTransportationNotifiaction()
        {
            string title = $"Local Transportation is needed for {_activity.Title} ";
            string body = $" {_activity.CreatedBy} has requested a Local Transportation Requirement {_activity.Title}. <p></p><p></p>";
            if (!string.IsNullOrEmpty(_hostingReport.ModeOfTravel))
            {
                body = body + $"<p><strong>Mode of Travel: </strong> {_hostingReport.ModeOfTravel} </p>";
            }
            if (!string.IsNullOrEmpty(_hostingReport.TravelArrangementDetails))
            {
                body = body + $"<p><strong>Travel Arrangement Details: </strong> {_hostingReport.TravelArrangementDetails} </p>";
            }
            body = body + GetHostingReportDetails() + $@"<p>
                                              to the view the event in the (EEM), click the: <a href='{_settings.BaseUrl}?id={_activity.Id}&categoryid={_activity.CategoryId}'> EEM Link </a> 
                                            <p>DO NOT REPLY TO THIS E-MAIL. THIS MESSAGE WAS AUTOMATICALLY GENERATED BY THE SYSTEM AND IS NOT MONITORED.</p>";

            await GraphHelper.SendEmail(GetEmails("Local Transportation Requirement"), title, body);
            var hostingReportToUpdate = await _context.HostingReports.FindAsync(_hostingReport.Id);
            hostingReportToUpdate.HostingReportLocalTransportationNotificationSent = true;
            _context.HostingReports.Update(hostingReportToUpdate);
            await _context.SaveChangesAsync();
        }
