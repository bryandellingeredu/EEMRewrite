...
export default function FullScreenMSFPCalendar (){
    const [view, setView] = useState(localStorage.getItem("calendarViewMSFP") || "dayGridMonth");
...
const  handleMouseEnter = async (arg : EventClickArg) =>{
  ...
  ${arg.event.extendedProps.educationalCategory ?'<p><strong>Educational Category: <strong>' + arg.event.extendedProps.educationalCategory + '</p>' : ''}
   `;
 ...
 {loadingEvent &&  <Modal.Header>
  ...
  <p> 
    <a href={eventInfo.hyperLink} className="ui orange button" target="_blank">
      ...
      {eventInfo.educationalCategory && <p><strong>Educational Category: </strong>{eventInfo.educationalCategory}</p>}
         ...
         <Loader active size='large' style={{marginTop: '100px'}}>
           ...
           ref={calendarRef}
            ...
            datesSet={(arg) => {
              ...
              localStorage.setItem("calendarViewMSFP", arg.view.type);
