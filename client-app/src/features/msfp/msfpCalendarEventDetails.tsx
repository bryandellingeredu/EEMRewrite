...
export default function MSFPCalendarEventDetails({eventInfo} : Props) {
    return(
        <>
        <SegmentGroup>
        <Segment>
  ...
  <Segment> 
    <a href={eventInfo.hyperLink} className="ui orange button" target="_blank">
      {eventInfo.hyperLinkDescription.length > 500 
       ...
       {eventInfo.educationalCategory &&  <Segment><strong> Educational Category: </strong>{eventInfo.educationalCategory}</Segment>}
        </SegmentGroup>
        </>
    )
}
