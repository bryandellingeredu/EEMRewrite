...
interface EventInfo
  ...
  export default function SpouseCalendarEventDetails({eventInfo, setShowDetailsFalse} : Props) {
    return(
        <>
        <SegmentGroup>
        <Segment>
  ...
  <Segment> 
    <a href={eventInfo.hyperLink} className="ui orange button" target="_blank">
      {eventInfo.hyperLinkDescription.length > 500 
       ...
       {eventInfo.spouseCategory &&  <Segment><strong> Educational Category: </strong>{eventInfo.spouseCategory}</Segment>}
       <Segment>
       <Button basic color="teal" onClick={setShowDetailsFalse} type='button' >
                        <Icon name="arrow left" /> Back To Calendar
                    </Button>
